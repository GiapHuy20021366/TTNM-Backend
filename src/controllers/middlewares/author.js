...
const authorCheckerForCreate = async (req, res, next) => {
  ...
  if (!name) {
    return res.status(400).send("No name found in request");
  }
  if (!alias) {
    return res.status(400).send("No alias found in request");
  }
  const aliasExisted = await authorService.findAuthorByAlias(alias);
  if (aliasExisted) {
    return res.status(400).send("Alias existed, please using another alias");
  }
  next();
};
